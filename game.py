...
class Game:
    ...
    def __init__(self, surface, array, tickrate = 0.1, cell_size = 20, rect = (30, 30, 600, 600), build_area = None):
        ...
        if build_area:
            ...
            # self.build_area = build_area # 3-deep tuple of rectangles defined by coordinate pairs in the format (((y,x), (y,x)), ((y,x), (y,x))) that player can build in
        else:
    ...
    def check_bases(self):
        ...
        if np.any(self.grid.damaged_base):
            ...
            damaged = [(np.nonzero(self.grid.damaged_base)[0][n], np.nonzero(self.grid.damaged_base)[1][n]) for n in range(len(np.nonzero(self.grid.damaged_base)))]
            for cell in damaged:
...
class LevelEditor(Game):
    ...
    def __init__(self, surface, array):
        Game.__init__(self, surface, array, build_area=(((0,0),(0,0)),))
